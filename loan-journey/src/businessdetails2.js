...
// const BusinesspersonalDetailsScreen2 = () => {
...
//     navigation.navigate('BusinesscommunicationDetailsScreen');
//   } else {
...
// export default BusinesspersonalDetailsScreen2;
...
const BusinesspersonalDetailsScreen2 = () => {
  ...
  const mapqualificationvalue = {
    "Trader/wholeseller" : "1",
    "Manufacturer" : "2",
    "Retailer" : "3",
    "Service provider & others" : "4"
  }
  ...
  const fetchHighestQualificationList = async () => {
    try {
      const response = await fetch('https://prod.utils.buddyloan.in/master_values.php/?master_key=nature_of_business');
      const json = await response.json();
      if (json.status === 'success' && json.HTTPStatus === 200) {
        setHighestQualification(json.master_values.map(item => item.nature_of_business_name));
  ...
  const fetchGenderList = async () => {
    try {
      const response = await fetch('https://prod.utils.buddyloan.in/master_values.php/?master_key=years_in_business');
      const json = await response.json();
      if (json.status === 'success' && json.HTTPStatus === 200) {
        setGender(json.master_values.map(item => item.years_in_business_name));
      } else {
        // Handle the error according to your app's needs
        console.error('Failed to fetch bank list');
      }
  ...
  useEffect(() => {
    ...
    const index2 = parseInt(userArray[0].businessNatureId,10) -1;// Convert to integer and adjust index
    ...
    if (index2 >= 0 && index2 < HighestQualification.length) {
      ...
      setSelectedValue2("Nature of Business");
    }
    ...
    >
      ...
      >
        <Text style={styles.dropdownText}>
          {selectedValue ? selectedValue : 'No of years in business'}
        </Text>
  ...
  <Text style={styles.errorMessage}>
    Please select your No of years in business
  </Text>
)}
    </View>
    <View style={styles.container1}>
      ...
      >
        <Text style={styles.dropdownText}>
          {selectedValue2 ? selectedValue2: 'Nature of Business'}
        </Text>
        {/* <Text style={styles.dropdownArrow}>▼</Text> */}
        <Image
          source={require('../assets/images/downarrow.png')}
      ...
      >
        ...
        >
          ...
          <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>Nature of Business</Text>
...
<TouchableOpacity onPress={() => {
  ...
  if (!selectedValue || selectedValue == "No of years in business") {
    // Step 2: Check if a gender has been selected and show error if not
    setShowGenderError(true);
    console.log("Please select your no of years in business");
  }
  if (!selectedValue2 || selectedValue2 == "Nature of Business") {
    ...
    // save_partial_loandata('emp_exp_month',user.empExpMonth,user);
    // save_partial_loandata('business_nature',user.business_nature_id,user);
    savePartialLoanData('emp_exp_month',gendermappedvalue,mobileNum);
    savePartialLoanData('business_nature',qualificationmappedvalue,mobileNum);

    navigation.navigate('BusinesspersonalDetailsScreen');
