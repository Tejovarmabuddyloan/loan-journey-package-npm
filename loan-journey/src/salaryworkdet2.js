...
const SalaryworkDetailsScreen2 = () => {
 let index;
  const navigation = useNavigation();
  intialDesignation = userArray.length > 0 ? userArray[0].designation : '';
  ...
  const handleForwardClick = () => {
   ...
   console.log('Loan amouasnt:', email);
    ...
    if (isLoanAmountValid && isDropdownValid) {
      ...
      savePartialLoanData('emp_exp_month', emp_exp_monthTypeMapping[selectedValue],mobileNum);
      savePartialLoanData('designation', email,mobileNum);
      ...
      if (!isDropdownValid) {
        setDropdownError('Please select the number of years in the current company');
      }
    }
  };
  ...
  const fetchCompanyList = async () => {
    try {
      const response = await fetch('https://prod.utils.buddyloan.in/master_values.php/?master_key=work_years_company');
      const json = await response.json();
      if (json.status === 'success' && json.HTTPStatus === 200) {
        setCompanyYears(json.master_values.map(item => item.work_years_company_name));
      } else {
  ...
  // }, [CompanyYears]); 
  useEffect(() => {
    const initialCurrentAddrMonth = userArray.length > 0 ? userArray[0].empExpMonth : '';
    const index2 = userArray.length > 0 ? Math.max(0, parseInt(userArray[0].empExpMonth, 10) - 6) : 0;
...
export default SalaryworkDetailsScreen2;
